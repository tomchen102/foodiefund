...
export function createMetadata({ title, description, keywords, images }: createMetadataProps) {
  const defaultImage = {
  ...
  const image = images ? { ...defaultImage, ...images } : defaultImage;

  return {
    title: title ? `${title} | 眾資成城-在你心愛的餐廳成為合夥人` : "眾資成城-在你心愛的餐廳成為合夥人",
    description:
      description ||
    ...
    keywords: keywords || "眾資成城, 餐廳, 合夥人, 群眾募資, 美食",
    openGraph: {
      title,
      description,
      type: "website",
      siteName: title ? `${title} | 眾資成城-在你心愛的餐廳成為合夥人` : "眾資成城-在你心愛的餐廳成為合夥人",
      images: [image],
    },
    twitter: {
